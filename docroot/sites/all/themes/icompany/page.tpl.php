...
<?php
// user box
function overriden_icompany_welcome_user(){
  global $user;
  $usr_path = 'user/'.$user->uid;
  $myAccount = drupal_get_path_alias($usr_path);
  $logout = drupal_get_path_alias('caslogout');
  if($user->uid)
  {
            ...
            <div id="top-bar-inner">
                ...
                <div class="span5 loginlinks">
                    <?php if(theme_get_setting('use_login_links') == 1) { ?>
                        <div id="user_links">
                            <?php if (!($user->uid)) { print overriden_user_login_links(); } else { print  overriden_icompany_welcome_user(); } ?>
                        </div>
                    <?php } ?>
                </div>
  ...
  $tabs_exist = region_blocks_exist($tab1,$tab2,$tab3,$tab4,$tab5,$tab6 );	?>
        ...
        <?php if(($title) && !(upperRegionsExist($upper_blocks1, $upper_blocks2, $upper_blocks3, $upper_blocks4))){ ?>
            <div id="title-region" class="row-fluid">
                ...
                <div class="span3 pagesearch"  style="float:right">
                    <!--Search Block -->
